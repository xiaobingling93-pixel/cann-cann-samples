...
/*!
 * \file dispatch_and_combine_non_quant.h
 * \brief
 */
#ifndef MOE_DISTRIBUTE_DISPATCH_NON_QUANT_H
#define MOE_DISTRIBUTE_DISPATCH_NON_QUANT_H
...
public:
    ...
    using ExpandXOutType = float16_t;
...
private:
    ...
    __aicore__ inline void TokenToExpert(int32_t toRankId, GlobalTensor<ExpandXOutType> dstWinGMTensor, TQue<QuePosition::VECIN, 1> inQueue,
                                                    uint32_t srcTokenIndex, uint32_t toExpertId);
    ...
    __aicore__ inline int64_t GetShmemSignalAddr(__gm__ uint8_t *shmemSpace, int32_t pe) {
        return (int64_t) aclshmem_ptr(shmemSpace, pe) + STATUS_REGION_OFFSET;
    }
    ...
    __aicore__ inline GM_ADDR GetWindStateAddrByRankId(const int32_t rankId) {
        return (GM_ADDR)GetShmemSignalAddr(shmemContextGM_, rankId) + dataState_ * WIN_STATE_OFFSET;
    }

    __aicore__ inline GM_ADDR GetSymMemDataRegionAddr()
    {
        return shmemContextGM_ + winDataSizeOffset_;
    }

    __aicore__ inline GM_ADDR GetSymMemStatusRegionAddr()
    {
        return shmemContextGM_ + STATUS_REGION_OFFSET + dataState_ * WIN_STATE_OFFSET;
    }
    ...
    __aicore__ inline uint32_t InitWinState(GlobalTensor<uint32_t> selfDataStatusGMTensor, TBuf<> dataStateBuf)
    {
        ...
        dataStateLocalTensor.SetValue(MOE_NUM_POS, moeExpertNum_);
        dataStateLocalTensor.SetValue(TILING_WORLDSIZE_POS, epWorldSize_);
        dataStateLocalTensor.SetValue(GLOBALBS_POS, globalBS_);
    ...
    const DispatchTilingData *tilingData)
{
    ...
    // FP16输入缓冲区：128个数据对齐
    hAlignSize_ = Align128(axisH_) * sizeof(XType);
    // FP16输出区：32字节UB对齐
    hOutSizeAlign_ = Ceil(axisH_ * sizeof(XType), UB_ALIGN) * UB_ALIGN;
    // 总内存大小=输出区对齐后的大小:hOutSizeAlign_
    tokenOffsetAlign_ = hOutSizeAlign_ / sizeof(int32_t);
...
__aicore__ inline void MoeDistributeDispatch::Init(GM_ADDR shmemSpace, GM_ADDR x, 
    GM_ADDR expertIds, GM_ADDR expandXOut,
    ...
    LocalTensor<ExpandXOutType> &xOutTensor, uint32_t tokenIndex, uint32_t k)
{
    ...
    xOutTint32(tokenOffsetAlign_) = epRankId_;        // 0:epRankId index
    xOutTint32(tokenOffsetAlign_ + 1) = tokenIndex;   // 1:token index
    xOutTint32(tokenOffsetAlign_ + 2) = k;            // 2:topK value index
    SyncFunc<AscendC::HardEvent::S_MTE3>();
}

__aicore__ inline void MoeDistributeDispatch::TokenToExpert(int32_t toRankId, GlobalTensor<ExpandXOutType> dstWinGMTensor,
        ...
        for (int32_t maskIndex = 0; maskIndex < maskN64Num; maskIndex++) {
            ...
            while (curValidIdx >= 0) {
                ...
                if (!((expertIndex == sendNum_ - 1) && (remainderExpertNum_ != 0) && (dstTokenPreCnt % delCurExpertGroupNum != groupIdx))) {
                    TokenToExpert(toRankId, dstWinGMTensor, inQueue, srcTokenIndex, topKIndex);
                }
...
__aicore__ inline void MoeDistributeDispatch::AllToAllDispatch()
{
    ...
    TBuf<> tokenNumToExpertBuf, receiveDataCastFloatBuf;
    ...
    LocalTensor<int32_t> xOutInt32Tensor, GM_ADDR wAddr, uint32_t index, uint32_t dstPosition, uint32_t arriveCount)
{
    ...
    DataCopyPad(expandIdxGMTensor_[dstPosition * EXPAND_IDX_INFO], xOutInt32Tensor[tokenOffsetAlign_],
                expandIdxCopyParams);
}
...
__aicore__ inline void MoeDistributeDispatch::ExpIdsCopyAndMaskCal()
{
