...
#include "utils.h"
#include "../include/moe_distribute_dispatch_non_quant.h"
...
int runDispatchAndCombine(int rankNum, int rankId, int bs, uint64_t shmemSpaceSize) {
    ...
    INFO_LOG("using aiv core num: %u", aivCoreNum);      
    
    // shmem init
    uint64_t localMemSize = shmemSpaceSize;
    ...
    int32_t aclshmemSize = shmemSpaceSize;
    ...
    SetDispatchTilingData(dispatchTilingData, rankNum, rankId, bs, aivCoreNum, shmemSpaceSize);
    ...
    size_t expandXOutSize = maxReceivedTokenNum * dispatchTilingData.h * sizeof(float16_t);
    InitData(&expandXOutHost, &expandXOutDevice, expandXOutSize);

    size_t dynamicScalesSize = 1;
    ...
    SetCombineTilingData(combineTilingData, rankNum, rankId, bs, aivCoreNum, shmemSpaceSize);
...
int main(int argc, char* argv[])
{
    ...
    int bs = atoi(argv[2]);
    uint64_t shmemSpaceSize = 1024UL * 1024UL * 1024UL; // 默认 1GB
    if (argc >= 4) {
    ...
    for (int rankId = 0; rankId < rankNum; ++rankId) {
        ...
        if (pid < 0) {
            ...
            int ret = runDispatchAndCombine(rankNum, rankId, bs, shmemSpaceSize);
